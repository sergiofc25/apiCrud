...
namespace BlazorAppPreWeb.HttpRepository.Repository
{
    public class HttpUsuarioRepository : IHttpUsuarioRepository
    {
        ...
        public HttpUsuarioRepository(HttpClient httpClient, ILocalStorageService localStorage)
        {
        ...
        public async Task<PagingResponse<Ent_Usuario>> Obten_Paginado(ClienteParameters parameters)
        {
            var queryStringParam = new Dictionary<string, string>
            {
                ["NumeroPagina"] = parameters.NumeroPagina.ToString(),
                ["RegistroPagina"] = parameters.RegistroPagina.ToString(),
                ["PorCorreo"] = parameters.SearchTerm ?? ""
            };
            using var response =
                await _httpClient.GetAsync(
                    QueryHelpers.AddQueryString("api/v1/Usuario", queryStringParam));
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                {
                    Cuerpo = await JsonSerializer.DeserializeAsync<List<Ent_Usuario>>(Cuerpo, _options),
                    MetaData = Metadata
                };
